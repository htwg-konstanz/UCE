...
/**
 * This class is part of the ConnectionReversalTarget class. It sends every
 * iterationTime a keep alive message to mediator.
 * 
...
public class KeepAliveThread extends Thread {
    ...
    public KeepAliveThread(String userName, final Socket socketToMediator, int iterationTimeInSeconds) {
    ...
    public void run() {
        ...
        UceMessage uceMessage = UceMessageStaticFactory.newUceMessageInstance(CommonUceMethod.KEEP_ALIVE,
                SemanticLevel.REQUEST, UUID.randomUUID());

        try {
            uceMessage.addAttribute(new UniqueUserName(userName));
            // byte[] buf = uceMessage.toByteArray();
            // datagrammPacket = new DatagramPacket(buf, buf.length,
            // socketAddress);
            ...
            while (!isInterrupted()) {
                try {
                    ...
                    uceMessage.writeTo(this.socketToMediator.getOutputStream());
